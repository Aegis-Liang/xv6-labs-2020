    ...
    while(buf[i] != '\0')
    {
      ...
      if(buf[i]==' ' || buf[i]=='\n')
      {
        ...
        for(int k=1; k<argc; k++)
        {
           ...
           // add last new argument
           if(k==argc-1)
             new_argument[k] = buf_argument[buf_argument_index];
        }
        ...
        if(fork()==0) // fork() here since the exec() will not return to other loop, I cannot fork() 1 time but exec() many times
        {
          // child process
          // exec
